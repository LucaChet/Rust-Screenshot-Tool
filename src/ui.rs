use chrono;
use druid::widget::{
    Button, Checkbox, ClipBox, Controller, Either, Flex, Image, Label, LineBreaking, List, Padding,
    Painter, RadioGroup, Svg, SvgData, TextBox, ZStack,
};
use druid::{
    commands, Code, Color, Data, Env, Event, EventCtx, FileDialogOptions, FileSpec, Lens,
    ...
    Flex::column()
        ...
        .with_child(
            ...
            //     .lens(Screenshot::name))
                ...
                .with_child(
                    Button::new("Modifica").on_click(|_ctx, data: &mut Screenshot, _env| {
                        let dialog = 
                            TextBox::new()
                                .lens(Screenshot::new_name)
                                .controller(Enter {})
                                // .on_activate(move |_ctx, data: &mut Screenshot, _env| {
                                //     // Quando l'utente preme Invio nella casella di testo,
                                //     // aggiorna il testo del label e chiudi la finestra modale
                                //     data.set_label_text(data.new_label_text.clone());
                                //     ctx.submit_command(druid::commands::CLOSE_WINDOW);
                                // });
                        
                        ctx.submit_command(druid::commands::SHOW_DIALOG.with(dialog));
                    }),
                )
